...
namespace TireTraxLib
{
    public class DbManager : IDisposable
    {
        ...
        {
            return new DbManager(System.Configuration.ConfigurationManager.ConnectionStrings["TireTraxConnectionString"].ConnectionString);
        }
        ...
        #region ExecuteScalar
        /// <summary>
        /// Updated Execute procedure and returns single value returned by it.
        /// Edited:sheraz,442011
        /// <returns> int containing value returned by user. </returns>
        ///  <summary>
        public T ExecuteScalar<T>(string procName, SqlParameter[] prams)
        {
            try
            {
                if (procName == "")
            ...
            try
            {
                if (Value != null && Value.ToString() != string.Empty)
                {
                    ...
                    foreach (string s in arr)
                    {
                        ...
                        if (!String.IsNullOrEmpty(Conversion.ParseString(Value)))
                        {

                            if (Conversion.ParseString(Value).Contains(s))
                                Value = (object)(Conversion.ParseString(Value).Replace(s, ""));
        ...
        public SqlParameter MakeInParamWithoutFormat(string ParamName, SqlDbType DbType, int Size, object Value)
        {
            try
            {
                if (Value != null && Value.ToString() != string.Empty)
                {
                 ...
                 Value = (object)(Conversion.ParseString(Value));
                }
            }
            catch { }
