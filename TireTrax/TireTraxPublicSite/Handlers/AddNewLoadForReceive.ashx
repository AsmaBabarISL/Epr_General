<%@ WebHandler Language="C#" Class="AddNewLoadForReceive" %>

using System;
using System.Web;
using TireTraxLib;
public class AddNewLoadForReceive : IHttpHandler {
    public string LoadNumber, outputType, PONumber, InvoiceNumber, SealNumber, TrailerNumber, Weight, BillOfLandingNumber, TypeId, userOrganizationId, LoginMemberId;
    public void ProcessRequest(HttpContext context)
    {
        context.Response.ContentType = "text/xml";
        LoadNumber = PONumber = InvoiceNumber = SealNumber = TrailerNumber = Weight = BillOfLandingNumber = TypeId = userOrganizationId = LoginMemberId = string.Empty;
        ...
        {
            context.Response.Write("<TireTrax><ErrorCode>607</ErrorCode><Message> Please provide Weight.</Message></TireTrax>");
            return;
        }
        ...
        {
            context.Response.Write("<TireTrax><ErrorCode>608</ErrorCode><Message> Please provide Bill of Landing Number.</Message></TireTrax>");
            return;
        }
        ...
        {
            context.Response.Write("<TireTrax><ErrorCode>611</ErrorCode><Message> Please provide Output type.</Message></TireTrax>");
            return;
        }
        addnewLoad(context);
    }
    ...
    private void addnewLoad(HttpContext context)
    {
        ...
        loadId = TireTraxLib.Loads.InsertRecieveLoad(Conversion.ParseInt(TypeId), PONumber, InvoiceNumber, SealNumber, TrailerNumber, Weight, BillOfLandingNumber, Conversion.ParseInt(userOrganizationId),Conversion.ParseInt(LoginMemberId), barcodeId, str, LoadNumber,357);
        string output = "";
